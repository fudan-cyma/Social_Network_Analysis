...
getNetStats_Denormalized=function(net)
{
  degree_in = degree(net, mode = "in", normalized = FALSE)
  degree_out = degree(net, mode = "out", normalized = FALSE)
  close_all = closeness(net, mode = 'all', normalized = FALSE)
...
```{r}
graph_1 = matrix(c(2,2,2,2,1,3,4,5),4,2)
...
```{r}
getNetStats_Denormalized(g1)
getNetStats_Denormalized(g0)

```


It's true for the graph with network-level centraility = 0, but not for the graph with network-level centraility = 1. 